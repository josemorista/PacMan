...
public:
    ...
    void Desenha(SDL_Renderer *render=NULL){
        if (render==renderer||render==NULL){
            ...
            SDL_Texture *textAux = SDL_CreateTextureFromSurface(render,bitmap);
            SDL_RenderCopyEx(render, textAux, &frame,&dest,-angulo,&pivoRelativo,flip);
    ...
    int Colisao(CObjeto* outro){
        SDL_Rect resp;
        return SDL_IntersectRect(&dest,&(outro->dest),&resp);
    }
